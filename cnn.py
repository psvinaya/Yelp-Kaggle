...
def load_dataset(n_imgs_train, n_imgs_val):
    train_file = "../input/processed/4/train_photo_to_attr.csv"
    val_file = "../input/processed/4/val_photo_to_attr.csv"
    dirname = "../input/train_photos/"

    def load_data(filename, n_imgs):
        data = pd.read_csv(filename)
        photo_ids = data.photo_id.tolist()
        y = np.array(data.attr.tolist(), dtype=np.int32)[0:n_imgs]
        X = np.zeros((n_imgs, 3, 64, 64), dtype=np.float64)
...
def main(num_epochs=10):
    # Load the dataset
    print "Loading data..." 
    X_train, y_train, X_val, y_val = load_dataset(50000, 5000)

    print "Preparing Theano"
    ...
    for epoch in range(num_epochs):
        ...
        filename = ''.join([str(epoch),'out.txt'])
        f = file(filename, 'w')
        output = lasagne.layers.get_output(network, X_val)
        print >> f, (T.argmax(output, axis = 1).eval())
        f.close()
